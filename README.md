## 🌍 ترجمة نافذة "تحسين البطارية" إلى جميع اللغات الـ31 — 28 يوليو 2026

### المشكلة
كانت نافذة "تحسين البطارية" (Battery Optimization Modal) في شاشة الإشعارات تُعرض باللغة العربية فقط لجميع المستخدمين، بغض النظر عن لغة التطبيق المختارة. كان النص التالي مثبتاً (hardcoded) في `NotificationManager.tsx`:
- عنوان النافذة: "لضمان وصول التنبيهات"
- الوصف: "أنظمة الهواتف قد تقوم بإيقاف التنبيهات لتوفير البطارية..."
- خطوات التفعيل وزرّا "السماح بالعمل في الخلفية" و"لاحقاً"

### الحل المُنفَّذ

#### 1. إضافة مفاتيح ترجمة جديدة (`i18n/translations.ts`)
تم إضافة 8 مفاتيح جديدة للواجهة `Translations`:
- `batteryOptTitle`, `batteryOptDesc`, `batteryOptStepsTitle`
- `batteryOptStep1`, `batteryOptStep2` (تدعمان HTML `<b>` للخط العريض)
- `batteryOptDontShow`, `batteryOptAllowBg`, `batteryOptLater`

#### 2. ترجمة لجميع اللغات الـ31 (`src/assets/i18n/*.json`)
تم توفير ترجمات احترافية لكل اللغات: العربية، الإنجليزية، الإندونيسية، الماليزية، الأردية، البنغالية، التركية، الفارسية، الهوسا، الفرنسية، الإسبانية، الألمانية، الروسية، السواحيلية، الصينية، الكورية، اليابانية، البوسنية، الألبانية، الأوزبكية، الكازاخستانية، الكردية، الفيتنامية، التاغالوغية، الهندية، التاميلية، السنهالية، الأمهرية، اليوروبا، الأورومو، والكينيارواندا.

#### 3. تحديث المكون (`components/NotificationManager.tsx`)
- استبدال جميع النصوص العربية المثبتة بمفاتيح `t.*`
- استخدام `dangerouslySetInnerHTML` لعرض الخط العريض `<b>` في الخطوات
- جعل اتجاه النص (RTL/LTR) ديناميكياً حسب لغة المستخدم بدلاً من إجباره على `rtl`

#### 4. سكربت الأتمتة (`scripts/fix_battery_opt_translations.cjs`)
سكربت Node.js يضيف الترجمات لجميع ملفات اللغات بشكل آمن (لا يُغيّر المفاتيح الموجودة).

### النتيجة
✅ جميع النصوص تُعرض الآن بلغة المستخدم بدلاً من العربية المثبتة.
✅ دعم كامل لاتجاه RTL و LTR.
✅ لا توجد أخطاء TypeScript.

---

## 🚨 إصلاح شامل لنظام المنبه: الصوت العالي + الهز في الخلفية — 27 يوليو 2026

### المشاكل التي تم حلها
1. **الصوت غير مرتفع**: كان صوت المنبه منخفضاً ولا يُسمع بوضوح حتى لو كان صوت الهاتف متوسطاً.
2. **الهز لا يعمل في الخلفية**: كشف الهز كان معتمداً على `devicemotion` في الويب، والذي يتوقف فور إخفاء التطبيق.
3. **عدم بدء خدمة الصوت تلقائياً**: المنبهات المجدولة كانت تُظهر إشعاراً عادياً فقط دون تشغيل خدمة الصوت الأصلية.

### الحلول المُنفَّذة

#### 1. تطوير خدمة `AlarmSoundService.java` (طبقة أندرويد الأصلية)
| التحسين | الوصف |
|---------|-------|
| **رفع الصوت للحد الأقصى** | `maximizeAlarmVolume()` يرفع مستوى `STREAM_ALARM` برمجياً إلى الحد الأقصى قبل التشغيل، ويُعيده لقيمته الأصلية عند الإيقاف |
| **قناة عالية الأولوية** | تغيير `IMPORTANCE_LOW` → `IMPORTANCE_HIGH` + `setBypassDnd(true)` لتجاوز وضع عدم الإزعاج |
| **اهتزاز نشط قوي** | إضافة `Vibrator` بنمط متكرر قوي `{0, 800, 400, 800, 400, 800, 400}` لجذب الانتباه |
| **WakeLock** | `PARTIAL_WAKE_LOCK` لإبقاء المعالج نشطاً أثناء المنبه (مهلة 10 دقائق) |
| **كشف هز محسّن** | رفع العتبة إلى `20.0f` + إضافة `debounce` (1.5 ثانية) لمنع التكرار الخاطئ |
| **أولوية الإشعار** | `PRIORITY_HIGH` + `CATEGORY_ALARM` + `VISIBILITY_PUBLIC` |

#### 2. ربط الإشعارات بخدمة الصوت (`App.tsx`)
| الملف | التغيير |
|-------|---------|
| `App.tsx` | في مستمع `localNotificationReceived`: فحص `extra.isAlarm` و `extra.sound`، وإذا كان منبهاً → تشغيل `AlarmSound.start()` تلقائياً + بناء `activeAlarm` لعرض شاشة المنبه |

#### 3. تمرير بيانات المنبه في الإشعارات (`NotificationManager.tsx`)
| الملف | التغيير |
|-------|---------|
| `NotificationManager.tsx` | إضافة `isAlarm` و `sound` و `name` إلى `extra` في جميع أنواع الجدولة (مرة واحدة، يومي، أسبوعي) |

### النتيجة
- ✅ **صوت مرتفع جداً**: يتم رفع مستوى الصوت للحد الأقصى تلقائياً ثم إعادته لقيمته الأصلية
- ✅ **اهتزاز قوي متكرر**: اهتزاز نشط بجانب الصوت لجذب الانتباه
- ✅ **الهز يعمل في الخلفية**: خدمة أندرويد أصلية تكشف الهز حتى لو كان التطبيق مغلقاً
- ✅ **تجاوز وضع الصامت**: استخدام `STREAM_ALARM` + `setBypassDnd(true)`
- ✅ **بدء تلقائي**: المنبه المجدول يُشغّل خدمة الصوت فور وصوله

### كيفية كتم المنبه
1. **الهز**: اهز الجهاز بقوة (يعمل في المقدمة والخلفية)
2. **أزرار الصوت**: الضغط على زر رفع أو خفض الصوت
3. **زر الإيقاف**: الضغط على زر "إيقاف" في شاشة المنبه

### الملفات المعدّلة
| # | الملف | نوع التعديل |
|---|-------|-------------|
| 1 | `android/app/src/main/java/com/mushafalmurajaa/app/AlarmSoundService.java` | تطوير شامل (رفع الصوت، اهتزاز، WakeLock، كشف هز محسّن) |
| 2 | `App.tsx` | ربط الإشعارات بخدمة الصوت الأصلية |
| 3 | `components/NotificationManager.tsx` | تمرير `isAlarm` و `sound` في بيانات الإشعار |
