...
const SKIP_SURAHS = new Set([2, 3, 4]); // Skip Baqarah (self), Imran, and Nisa (already done)
const MIN_WORDS = 4;
const MAX_WORDS = 20;
...
const APP_DATA_PATH = path.join(__dirname, '../src/data/custom_mutashabihat/baqarah_rest_generated.txt');
...
async function runAnalysis() {
    ...
    const baqarah = quranData.data.surahs.find(s => s.number === SOURCE_SURAH);
    const otherSurahs = quranData.data.surahs.filter(s => !SKIP_SURAHS.has(s.number));

    function prepareAyah(ayah) {
        let text = ayah.text;
        // Strip Bismillah from the first ayah if it exists (except Fatiha where it's part of the verse counts)
        if (ayah.numberInSurah === 1 && text.startsWith("بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ")) {
            // Only strip if there's more text after it, to avoid empty ayahs
            if (text.length > 39) {
    ...
    console.log("🛠️ Preparing Al-Baqarah Ayahs...");
    const sourceAyahs = baqarah.ayahs.map(prepareAyah);

    const matches = [];

    console.log(`🚀 Starting Strict Analysis against ${otherSurahs.length} Surahs...`);
    console.time("analysis");

    for (const targetSurah of otherSurahs) {
        // console.log(`Processing Surah: ${targetSurah.name}...`);
    ...
    const uniqueMatches = [];
    const usedRanges = new Map(); // "surah:ayah" -> Array of {start, end}

    for (const m of matches) {
        const sKey = `S${m.source.surah}:A${m.source.ayah}-T${m.target.surah}:A${m.target.ayah}`;
