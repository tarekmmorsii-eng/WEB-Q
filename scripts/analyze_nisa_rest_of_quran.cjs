...
const APP_DATA_PATH = path.join(__dirname, '../src/data/custom_mutashabihat/nisa_rest_generated.txt');
...
async function runAnalysis() {
    ...
    console.log("🛠️ Preparing Source Ayahs (An-Nisa)...");
    const sourceAyahs = sourceSurahData.ayahs.map(prepareAyah);

    const matches = [];

    console.log(`🚀 Starting Strict Analysis for An-Nisa against ${otherSurahs.length} Surahs...`);
